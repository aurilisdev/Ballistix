...
public class CapabilityActiveSAMs implements ICapabilitySerializable<CompoundNBT> {
	...
	public CompoundNBT serializeNBT() {
        ...
        for (Map.Entry<RegistryKey<World>, HashMap<UUID, VirtualProjectile.VirtualSAM>> entry : activeSAMs.entrySet()) {
            ...
            for (VirtualProjectile.VirtualSAM missile : entry.getValue().values()) {
                ...
                VirtualProjectile.VirtualSAM.CODEC.encode(missile, NBTDynamicOps.INSTANCE, new CompoundNBT()).result().ifPresent(tag -> stored.put("" + index, tag));
	...
	public void deserializeNBT(CompoundNBT nbt) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < activeSize; j++) {
                ...
                VirtualProjectile.VirtualSAM virtual = VirtualProjectile.VirtualSAM.CODEC.decode(NBTDynamicOps.INSTANCE, stored.getCompound("" + j)).result().get().getFirst();
