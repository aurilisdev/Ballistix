...
public abstract class VirtualProjectile {
	...
	@Override
	public void onHitMissile(Level world, VirtualMissile missile) {
	    if (Voltaic.RANDOM.nextDouble() < (variant == 0 ? BallistixConstants.SAM_CHANCE_TO_DESTROY
		    : BallistixConstants.ANTIBALLISTICMISSILE_CHANCE_TO_DESTROY)) {
		MissileManager.removeMissile(world.dimension(), missile.getId());
	    }
	    world.explode(null, null, null, position.x, position.y, position.z, 2.0F, false, BlockInteraction.BREAK);
	}
	...
	public void updatePosition(ServerLevel level) {
	    ...
	    float minSpeed = variant == 0 ? topSpeed * BallistixConstants.SAM_MINTURNSPEED_PERC
		    : topSpeed * BallistixConstants.ANTIBALLISTICMISSILE_MINTURNSPEED_PERC;
	    ...
	    double maxTurnRadians = variant == 0 ? BallistixConstants.SAM_ENTITY_TURNINGSPEEDRADIANS
		    : BallistixConstants.ANTIBALLISTICMISSILE_ENTITY_TURNINGSPEEDRADIANS;
