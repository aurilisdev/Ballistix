...
public class RenderMinecart extends EntityRenderer<EntityMinecart> {
    ...
    @Override
    public void render(EntityMinecart entity, float yaw, float partial, PoseStack stack, MultiBufferSource source,
	    int light) {
	...
	if (f5 > 0.0F) {
	    stack.mulPose(MathUtils.rotVectorQuaternionRad(Mth.sin(f5) * f5 * f6 / 10.0F * entity.getHurtDir(),
		    MathUtils.XP));
	    // stack.mulPose(Vector3f.XP.rotationDegrees(Mth.sin(f5) * f5 * f6 / 10.0F *
	    // entity.getHurtDir()));
	}
    ...
    protected void renderMinecartContents(EntityMinecart entity, float p1, BlockState state, PoseStack stack,
	    MultiBufferSource source, int p2) {
