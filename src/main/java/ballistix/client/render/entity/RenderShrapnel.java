...
public class RenderShrapnel extends EntityRenderer<EntityShrapnel> {
    ...
    public void vertex(Matrix4f matrix, Matrix3f normals, VertexConsumer consumer, int pOffsetX, int pOffsetY,
	...
	consumer.vertex(matrix, pOffsetX, pOffsetY, pOffsetZ).color(255, 255, 255, 255).uv(pTextureX, pTextureY)
		.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pPackedLight).normal(normals, pNormalX, nx, ny)
		.endVertex();
    }
