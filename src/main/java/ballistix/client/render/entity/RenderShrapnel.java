...
public class RenderShrapnel extends EntityRenderer<EntityShrapnel> {
    ...
    @Override
    public void render(EntityShrapnel entityShrapnel, float yaw, float partialticks, PoseStack stack,
	    MultiBufferSource buffer, int light) {
	stack.pushPose();
	stack.mulPose(MathUtils.rotVectorQuaternionDeg(
		Mth.lerp(partialticks, entityShrapnel.yRotO, entityShrapnel.getYRot()) - 90.0F, MathUtils.YP));
	stack.mulPose(MathUtils.rotVectorQuaternionDeg(
		Mth.lerp(partialticks, entityShrapnel.xRotO, entityShrapnel.getXRot()), MathUtils.ZP));
	stack.mulPose(MathUtils.rotVectorQuaternionDeg(45.0F, MathUtils.XP));
	// stack.mulPose(Vector3f.YP.rotationDegrees(Mth.lerp(partialticks,
	// entityShrapnel.yRotO, entityShrapnel.getYRot()) - 90.0F));
	// stack.mulPose(Vector3f.ZP.rotationDegrees(Mth.lerp(partialticks,
	// entityShrapnel.xRotO, entityShrapnel.getXRot())));
    ...
    public void vertex(Matrix4f matrix, PoseStack.Pose pose, VertexConsumer consumer, int pOffsetX, int pOffsetY,
	    int pOffsetZ, float pTextureX, float pTextureY, int pNormalX, int ny, int nx, int pPackedLight) {
	consumer.addVertex(matrix, pOffsetX, pOffsetY, pOffsetZ).setColor(Color.WHITE.color())
		.setUv(pTextureX, pTextureY).setOverlay(OverlayTexture.NO_OVERLAY).setLight(pPackedLight)
		.setNormal(pose, pNormalX, nx, ny);
    }
