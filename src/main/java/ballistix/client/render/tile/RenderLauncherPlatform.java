...
public class RenderLauncherPlatform<T extends GenericTile & ILauncherPlatform> extends AbstractTileRenderer<T> {
	...
	public RenderLauncherPlatform(TileEntityRendererDispatcher context) {
		super(context);
	}
	...
	public void render(T tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
		...
		} else if (stack.getItem() == BallistixItems.ITEM_AAMISSILEMK2.get()) {

			matrixStackIn.pushPose();

			IBakedModel model = Minecraft.getInstance().getModelManager().getModel(BallistixClientRegister.MODEL_AAMISSILE_MK2);
