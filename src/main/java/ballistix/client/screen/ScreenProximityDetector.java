...
public class ScreenProximityDetector extends GenericScreen<ContainerProximityDetector> {
	...
	addComponent(new ScreenComponentElectricInfo(-AbstractScreenComponentInfo.SIZE + 1, 2)
		.wattage(BallistixConstants.SAM_TURRET_USAGEPERTICK * 20));
	...
	addComponent(toggleLines = new ScreenComponentButton<>(92, 145, 70, 20).setLabel(() -> {
		...
		if (HandlerDetectorLines.containsLines(pos)) {
		    ...
		    AABB box = new AABB(detector.getBlockPos().offset(detector.minCorner.getValue().multiply(-1)),
			    detector.getBlockPos().offset(detector.maxCorner.getValue()).offset(1, 1, 1));
