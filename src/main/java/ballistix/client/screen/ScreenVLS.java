...
public class ScreenVLS extends GenericScreen<ContainerVLS> {
    ...
    public ScreenVLS(ContainerVLS container, PlayerInventory inv, ITextComponent title) {
        ...
        addComponent(new ScreenComponentSimpleLabel(101, 39, 10, Color.TEXT_GRAY, BallistixTextUtils.gui("missilesilo.sync")));

        addComponent(new ScreenComponentButton<>(130, 32, 20, 20).setOnPress(button -> {
            //
            TileVerticalLaunchSilo silo = getMenu().getSafeHost();
            if(silo == null) {
                return;
            }
            silo.shouldLaunch.setValue(true);

        }).setColor(new Color(255, 0, 0, 255)).onTooltip((poseStack, component, mouseX, mouseY) -> renderTooltip(poseStack, BallistixTextUtils.tooltip("silo.launch"), mouseX, mouseY)));
    ...
    private void setSiloTargetX(String coord) {
        ...
        TileVerticalLaunchSilo silo = menu.getSafeHost();
    ...
    private List<? extends IReorderingProcessor> getElectricInformation() {
        ...
        list.add(BallistixTextUtils.tooltip("missilesilo.charge", ChatFormatter.getChatDisplayShort(el.getJoulesStored(), DisplayUnits.JOULES).withStyle(TextFormatting.GRAY), ChatFormatter.getChatDisplayShort(BallistixConstants.MISSILESILO_USAGE, DisplayUnits.JOULES).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
        list.add(VoltaicTextUtils.gui("machine.voltage", ChatFormatter.getChatDisplayShort(el.getVoltage(), DisplayUnits.VOLTAGE).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
