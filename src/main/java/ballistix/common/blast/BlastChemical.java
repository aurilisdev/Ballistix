				...
				for (int y = -radius; y <= radius; y++) {
					for (int z = -radius; z <= radius; z++) {
						if (x * x + y * y + z * z < radius * radius && world.random.nextDouble() < 1 / 20.0) {
							world.addParticle(new RedstoneParticleData(0.7f, 0.8f, 0, 5), position.getX() + x + 0.5 + world.random.nextDouble() - 1.0, position.getY() + y + 0.5 + world.random.nextDouble() - 1.0, position.getZ() + z + 0.5 + world.random.nextDouble() - 1.0, 0.0D, 0.0D, 0.0D);
						}
					}
				}
			}
