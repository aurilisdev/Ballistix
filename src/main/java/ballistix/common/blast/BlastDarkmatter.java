...
public class BlastDarkmatter extends Blast {
	...
	public void doPreExplode() {
		if (!world.isClientSide) {
			...
			thread.start();
			world.playSound(null, position, BallistixSounds.SOUND_DARKMATTER.get(), SoundCategory.BLOCKS, 1.0F, 1.0F);
		} 
	}
	...
	public boolean doExplode(int callCount) {
		if (!world.isClientSide) {
			...
			if (world.random.nextFloat() < 0.5) {
				world.explode(null, position.getX(), position.getY(), position.getZ(), 2, Mode.NONE);
			}
		}
		return false;
	}
