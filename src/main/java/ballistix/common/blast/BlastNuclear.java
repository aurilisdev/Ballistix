...
public class BlastNuclear extends BlastCalculating implements IHasCustomRender {
	...
	if (!world.isClientSide) {
	    ...
	    threadSimple = new ThreadSimpleBlast(world, position, (int) (Constants.EXPLOSIVE_NUCLEAR_SIZE * 2),
...
//				p.getY() + 0.5, p.getZ() + 0.5, 0, 0, 0);
	...
	if (ticksSinceBlastStart < 5) {
	    // Fireball
	    ParticleOptions particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 3f, -0.045f, 1000,
		    true, true, 200, 0.97);
	    ...
	    particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 2.5f, 0.045f, 1000, true,
		    0.99);
	    BlastThermobaric.spawnSurroundingParticles(particle, random, centerX, centerY, centerZ, 75, 0, 20,
		    initialSpeed, true);
	    // Centersmokes
	    initialSpeed = 2; // Increase/decrease to taste
	    particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 2.5f, 0.01f, 1000, true,
		    0.99);
	    BlastThermobaric.spawnSurroundingParticles(particle, random, centerX, centerY, centerZ, 75, 0, 20,
		    initialSpeed, true);
	    // Centersmokes
	    initialSpeed = 2; // Increase/decrease to taste
	    particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 2.5f, -0.015f, 1000, true,
		    0.97);
		...
		particle = new ParticleOptionBlastSmoke().setParameters(1.5f, 1.5f, 1.5f, 1f, 0, 150, false, 1);
	    ...
	    ParticleOptions particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 3f, -0.045f,
		    Mth.clamp(1000 - ticksSinceBlastStart, 1, 1000), true, 0.95);
	    BlastThermobaric.spawnSurroundingParticles(particle, random, centerX, centerY, centerZ, 1, -20, 20,
		    initialSpeed, true);
	    // Centerfire rising
	    initialSpeed = 0.7; // Increase/decrease to taste
	    particle = new ParticleOptionBlastSmoke().setParameters(1.0f, 1.0f, 1.0f, 3f, -0.045f, Mth.clamp(1000 - ticksSinceBlastStart, 1, 1000),
		    true, true, 500, 0.97);
	    BlastThermobaric.spawnSurroundingParticles(particle, random, centerX, centerY + 0.033f * ticksSinceBlastStart, centerZ, 1, -20, 20,
		    initialSpeed, true);
	}
    }
