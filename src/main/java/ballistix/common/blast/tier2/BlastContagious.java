...
public class BlastContagious extends Blast {
    ...
    public BlastContagious(Level world, BlockPos position, @Nullable  Entity owner, @Nullable Entity blastEntity) {
	...
	hasStarted = true;
	int radius = (int) BallistixConfig.INSTANCE.EXPLOSIVE_CONTAGIOUS_SIZE.getAsDouble();
		...
		for (int y = -radius; y <= radius; y++) {
			...
			if (add && x * x + y * y + z * z < radius * radius && world.random.nextDouble() < 1 / 10.0) {
			    Minecraft.getInstance().particleEngine.createParticle(
				    new DustParticleOptions(new Vector3f(0.5f, 0.4f, 0), 5), xPos, yPos, zPos, 0.0D,
				    0.0D, 0.0D);
			}
		    }
		}
	...
	if (!world.isClientSide) {
		...
		if (entity instanceof LivingEntity living) {
		    living.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 360, 2), owner);
		    living.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 360, 2), owner);
		    living.addEffect(new MobEffectInstance(MobEffects.HUNGER, 360, 3), owner);
		    living.addEffect(new MobEffectInstance(BallistixEffects.VIRUS, -1), owner);
	...
	return callCount > BallistixConfig.INSTANCE.EXPLOSIVE_CONTAGIOUS_DURATION.getAsDouble();
    }
