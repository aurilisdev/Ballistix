...
public class BlastSonic extends BlastLasting {
	...
	while (iterator.hasNext()) {
	    ...
	    if (state.isAir() || state.getDestroySpeed(world, p) < 0 || state.getDestroySpeed(world,
		    p) > BallistixConfig.INSTANCE.EXPLOSIVE_SONIC_MAXHARDNESS.getAsDouble() || state.liquid()) {
		continue;
	    }
