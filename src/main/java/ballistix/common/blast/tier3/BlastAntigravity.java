...
public class BlastAntigravity extends BlastLasting {
	...
	return ticksSinceBlastStart >= BallistixConstants.EXPLOSIVE_ANTIGRAVITY_CHUNKDURATION;
    }

    private void liftEntities() {
	int radius = BallistixConstants.EXPLOSIVE_ANTIGRAVITY_RADIUS;
	...
	for (Entity entity : world.getEntitiesOfClass(Entity.class, bounds)) {
	    ...
	    double lift = (entity instanceof LivingEntity ? LivingEntity.DEFAULT_BASE_GRAVITY : 0.04D)
		    * BallistixConstants.EXPLOSIVE_ANTIGRAVITY_GRAVITYFACTOR;
	...
	int radius = Math.max(1, (int) (BallistixConstants.EXPLOSIVE_ANTIGRAVITY_RADIUS * 0.8F));
	...
	for (int attempt = 0; attempt < 16; attempt++) {
	    ...
	    if (state.isAir() || state.getMaterial().isLiquid() || state.getDestroySpeed(world, pos) < 0) {
		continue;
	    }

	    BlockState above = world.getBlockState(pos.above());

	    if (!above.isAir() && !above.getMaterial().isLiquid()) {
		continue;
	    }
