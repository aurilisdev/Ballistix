...
public class BlastAntimatter extends BlastLasting implements IHasCustomRender {
	...
	if (ticksSinceBlastStart < 10) {
	    ...
	    ParticleUtilities.spawnParticleSphere(particle, x, y, z, 50, -90, 90,
		    BallistixConfig.INSTANCE.EXPLOSIVE_ANTIMATTER_RADIUS.get()
			    / BallistixConfig.INSTANCE.EXPLOSIVE_ANTIMATTER_DURATION.get() * 2.5,
		    true);
	}
	// Shockwave
	double spawnSize = 3;
	...
	if (dist < 3) {
	    ...
	    CameraShakeEffect effect = CameraShakeManager.createBlastSourcedEffect(
		    BallistixConfig.INSTANCE.EXPLOSIVE_ANTIMATTER_DURATION.get(), endSize, world.getGameTime(), pos);
