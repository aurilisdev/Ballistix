...
public class BlastEMP extends Blast implements IHasCustomRender {
    ...
    public void doPreExplode() {
        if (!world.isClientSide) {
            thread = new ThreadSimpleBlast(world, position, (int) BallistixConfig.INSTANCE.EXPLOSIVE_EMP_RADIUS.getAsDouble(), Integer.MAX_VALUE, null, getBlastType().id());
            thread.start();
    ...
    public boolean doExplode(int callCount) {
        ...
        hasStarted = true;
        if (pertick == -1) {
            pertick = (int) (thread.results.size() / BallistixConfig.INSTANCE.EXPLOSIVE_ANTIMATTER_DURATION.getAsDouble() + 1);
        ...
        while (cachedIterator.hasNext()) {
            ...
            if (entity != null) {
                for (Direction dir : Direction.values()) {
                    ...
                    ICapabilityElectrodynamic electro = world.getCapability(VoltaicCapabilities.CAPABILITY_ELECTRODYNAMIC_BLOCK, p, world.getBlockState(p), entity, dir);
