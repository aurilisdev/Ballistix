...
public class BlastEnder extends Blast implements IHasCustomRender {
	...
	for (int i = 0; i < BallistixConstants.EXPLOSIVE_ENDER_ENDERMANCOUNT; i++) {
	    ...
	    entity.setPos(
		    //
		    position.getX()
			    + world.random.nextIntBetweenInclusive((int) -BallistixConstants.EXPLOSIVE_ENDER_RADIUS,
				    (int) BallistixConstants.EXPLOSIVE_ENDER_RADIUS),
		    //
		    position.getY()
			    + world.random.nextIntBetweenInclusive((int) -BallistixConstants.EXPLOSIVE_ENDER_RADIUS,
				    (int) BallistixConstants.EXPLOSIVE_ENDER_RADIUS),
	...
	world.explode(null, position.getX() + 0.5, position.getY() + 0.5, position.getZ() + 0.5,
		(float) BallistixConstants.EXPLOSIVE_ENDER_RADIUS, Level.ExplosionInteraction.BLOCK);
