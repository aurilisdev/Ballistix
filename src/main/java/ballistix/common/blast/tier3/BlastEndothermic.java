...
public class BlastEndothermic extends BlastLasting implements IHasCustomRender {
	...
	if (pertick == -1) {
	    ...
	    pertick = (int) (thread.results.size() * 1.5
		    / BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_DURATION.getAsDouble() + 1);
	...
	while (iterator.hasNext()) {
	    ...
	    if (state.isAir() || !(state.getBlock() instanceof LiquidBlock)
		    && (state.getDestroySpeed(world, p) < 0 || state.getDestroySpeed(world,
			    p) > BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_MAXHARDNESS.getAsDouble())) {
		continue;
	    }
	...
	if (!iterator.hasNext()) {
	    ...
	    float size = (float) BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_RADIUS.getAsDouble();
		...
		double velX = deltaX * inverseMag
			* BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_VELOCITY.getAsDouble();
		double velY = deltaY * inverseMag
			* BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_VELOCITY.getAsDouble();
		double velZ = deltaZ * inverseMag
			* BallistixConfig.INSTANCE.EXPLOSIVE_ENDOTHERMIC_VELOCITY.getAsDouble();
		entity.setTicksFrozen(entity.getTicksRequiredToFreeze());
		entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 10000), owner);
		entity.addEffect(new MobEffectInstance(BallistixEffects.FROSTBITE, 10000), owner);
