...
public class BlastHypersonic extends BlastLasting {
    ...
    public BlastHypersonic(World world, BlockPos position) {
    ...
    public void doPreExplode() {
        if (!world.isClientSide) {
            ...
            thread.start();
            world.playSound(null, position, BallistixSounds.SOUND_HYPERSONICSONICEXPLOSION.get(), SoundCategory.BLOCKS, 25, 1);
        }
    }
    ...
    public boolean doExplode(int callCount) {
        ...
        while (iterator.hasNext()) {
            ...
            if(state.isAir() || state.getDestroySpeed(world, p) < 0 || state.getDestroySpeed(world, p) > BallistixConstants.EXPLOSIVE_HYPERSONIC_MAXHARDNESS || state.getBlock() instanceof IFluidBlock) {
                continue;
            }
