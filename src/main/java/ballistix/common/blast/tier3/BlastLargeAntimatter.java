...
public class BlastLargeAntimatter extends BlastLasting implements IHasCustomRender {
	...
	public boolean doExplode(int callCount) {
		...
		Explosion ex = new Explosion(world, null, null, null, position.getX(), position.getY(), position.getZ(), (float) BallistixConstants.EXPLOSIVE_LARGEANTIMATTER_RADIUS, false, Explosion.Mode.DESTROY);
		if (pertick == -1) {
			hasStarted = true;
		...
		while (iterator.hasNext()) {
			...
			if (!state.isAir() && state.getDestroySpeed(world, p) >= 0) {
				...
				if (world.random.nextFloat() < 1 / 80.0 && world instanceof ServerWorld) {
