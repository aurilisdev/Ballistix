...
public class BlastLargeAntimatter extends BlastLasting implements IHasCustomRender {
	...
	public boolean doExplode(int callCount) {
		...
		while (iterator.hasNext()) {
			...
			if (!state.isAir() && state.getDestroySpeed(world, p) >= 0) {
				...
				if (world.random.nextFloat() < 1 / 80.0 && world instanceof ServerLevel serverlevel) {
	...
	public void produceParticles() {
		...
		ParticleUtilities.spawnParticleSphere(particle, x, y, z, 100, -90, 90, BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_RADIUS.get() / BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_DURATION.get() * 2, true);
		// Shockwave
		double spawnSize = 3;
