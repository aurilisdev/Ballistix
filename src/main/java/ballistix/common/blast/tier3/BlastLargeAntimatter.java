...
public class BlastLargeAntimatter extends BlastLasting implements IHasCustomRender {
    ...
    public BlastLargeAntimatter(Level world, BlockPos position, Entity owner) {
	...
	if (!world.isClientSide) {
	    ...
	    thread.start();
	    world.playSound(null, position, BallistixSounds.SOUND_LARGE_ANTIMATTEREXPLOSION.get(), SoundSource.BLOCKS,
		    25, 1);
	}
    }
	...
	Explosion ex = new Explosion(world, null, null, null, position.getX(), position.getY(), position.getZ(),
		(float) BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_RADIUS.getAsDouble(), false,
		BlockInteraction.DESTROY, ParticleTypes.EXPLOSION, ParticleTypes.EXPLOSION_EMITTER,
		SoundEvents.GENERIC_EXPLODE);
	if (pertick == -1) {
	    hasStarted = true;
	    pertick = (int) (thread.results.size() * 1.5
		    / BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_DURATION.get() + 1);
	...
	if (ticksSinceBlastStart < 10) {
	    ...
	    ParticleUtilities.spawnParticleSphere(particle, x, y, z, 60, -90, 90,
		    BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_DURATION.get()
			    / BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_DURATION.get() * 2.5,
		    true);
	} // Shockwave
	double spawnSize = 3;
	...
	if (dist < 3) {
	    ...
	    CameraShakeEffect effect = CameraShakeManager.createBlastSourcedEffect(
		    BallistixConfig.INSTANCE.EXPLOSIVE_LARGEANTIMATTER_DURATION.get(), endSize, world.getGameTime(),
		    pos);
