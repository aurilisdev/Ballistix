...
public class DynamicRay {
    ...
    public DynamicRay(float currentX, float currentY, float currentZ, float dx, float dy, float dz, float power,
	    BlockPos currentBlockPos, float power_decrease, ThreadDynamicRaycastBlast mainBlast) {
    ...
    public boolean tick(final BlockPos position, final Level world, final IResistanceCallback callback,
	    final Entity explosionSource, HashMap<BlockPos, BlockState> alreadyDestroyed) {
	if (power > 0.0F) {
		...
		if (!alreadyDestroyed.containsKey(next)) {
			...
			if (!block.isAir()) {
				...
				power -= Math.max(power_decrease, callback.getResistance(world, position,
				...
				if (power > 0f) {
					...
					HashDistanceBlockPos hashedPos = new HashDistanceBlockPos(
		    ...
		    if (!block.isAir()) {
			if (block.getDestroySpeed(world, currentBlockPos) >= 0) {
			    power -= Math.max(power_decrease,
				    callback.getResistance(world, position, next, explosionSource, block));
			}
		    }
		}
		currentX += dx;
