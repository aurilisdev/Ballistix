...
public class EntityBlast extends Entity implements TraceableEntity {
	...
	if (!level().isClientSide() && !hasMatured) {
	    ChunkPos pos = level().getChunk(blockPosition()).getPos();
	    ForgeChunkManager.forceChunk((ServerLevel) level(), Ballistix.ID, blockPosition(), pos.x, pos.z, true,
		    true);
	}
    }
	...
	if (!level().isClientSide && reason == RemovalReason.DISCARDED && !hasMatured) {
	    ChunkPos pos = level().getChunk(blockPosition()).getPos();
	    ForgeChunkManager.forceChunk((ServerLevel) level(), Ballistix.ID, blockPosition(), pos.x, pos.z, false,
		    true);
	}
	super.remove(reason);
    }
