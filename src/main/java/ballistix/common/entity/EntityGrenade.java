...
public class EntityGrenade extends ThrowableProjectile implements IDefusable {
    ...
    private static final EntityDataAccessor<Integer> FUSE = SynchedEntityData.defineId(EntityGrenade.class,
	    EntityDataSerializers.INT);
    private static final EntityDataAccessor<String> TYPE = SynchedEntityData.defineId(EntityGrenade.class,
	...
	if (blastId != null) {
	    ...
	    ItemEntity item = new ItemEntity(level(), getBlockX() + 0.5, getBlockY() + 0.5, getBlockZ() + 0.5,
		    new ItemStack(explosive.getExplosiveItem().get()));
	...
	if (fuse <= 0) {
	    ...
	    if (blastId != null) {
		IBlast explosive = Blast.BLAST_MAP.get(blastId);
		// TODO: This is wrong. use getowner no matter what!!!
