...
public class EntityMissile extends Entity {
	...
	public void tick() {
		if (getMotion().length() > 0) {
			rotationPitch = (float) (Math.atan(getMotion().getY() / Math.sqrt(getMotion().getX() * getMotion().getX() + getMotion().getZ() * getMotion().getZ())) * 180.0D / Math.PI);
		...
		for (int i = 0; i < 5; i++) {
			...
			world.addParticle(ParticleTypes.LARGE_SMOKE, x + ranX, y + ranY, z + ranZ, -getMotion().x + ranX, -getMotion().y - 0.075f + ranY, -getMotion().z + ranZ);
		}
	}
