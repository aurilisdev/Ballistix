...
public class EntityMissile extends Entity {
	...
	if ((path == VirtualMissile.FlightPath.SILO || path == VirtualMissile.FlightPath.SILO_CLUSTER
		|| path == VirtualMissile.FlightPath.VLS && hasIgnighted)
		&& !target.equals(BlockEntityUtils.OUT_OF_REACH) && speed < 3.0F) {
	    speed += 0.02F;
	}

	if (missileType == -1 || isServerSide || speed >= 3.0F
		|| path == VirtualMissile.FlightPath.VLS && !hasIgnighted) {
	    return;
	}
	...
	BlockPos.CODEC.encode(target, NbtOps.INSTANCE, new CompoundTag()).result()
	...
	BlockPos.CODEC.decode(NbtOps.INSTANCE, compound.getCompound("target")).result()
		.ifPresent(pair -> target = pair.getFirst());
