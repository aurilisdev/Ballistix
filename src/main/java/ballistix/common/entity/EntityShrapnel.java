...
public class EntityShrapnel extends ThrowableProjectile {
	...
	if (isOnGround() || tickCount > (isExplosive ? 400 : 100)
		|| level.getBlockState(blockPosition()).getMaterial().blocksMotion()) {
	...
	if (isExplosive) {
	    level.explode(this, BallistixDamageTypes.SHRAPNEL, null, getX(), getY(), getZ(), 3, true,
