...
public class ItemGrenade extends Item {
	...
	if (!world.isRemote) {
	    world.playSound(null, entityLiving.getPosX(), entityLiving.getPosY(), entityLiving.getPosZ(),
		    SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 0.5F,
		    0.4F / (random.nextFloat() * 0.4F + 0.8F));

	    float throwEnergy = (float) (getUseDuration(itemStack) - timeLeft) / (float) getUseDuration(itemStack)
		    + 0.7f;

	    EntityGrenade grenade = new EntityGrenade(world);
	    grenade.setLocationAndAngles(entityLiving.getPosX(),
	    ...
	    grenade.func_234612_a_(entityLiving, entityLiving.rotationPitch - 20, entityLiving.rotationYaw, 0.0F,
		    throwEnergy, 1.0F);
