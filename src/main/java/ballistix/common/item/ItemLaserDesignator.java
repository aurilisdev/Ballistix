...
public class ItemLaserDesignator extends ItemElectric {
	...
	if (stack.hasTag() && stack.getTag().contains(FREQUENCY_KEY)) {
	    tooltip.add(BallistixTextUtils.tooltip("laserdesignator.frequency", getFrequency(stack))
	...
	tooltip.add(BallistixTextUtils
		.tooltip("laserdesignator.signalrange", Component
			.literal("" + BallistixConstants.LASER_DESIGNATOR_RANGE).withStyle(ChatFormatting.GRAY))
