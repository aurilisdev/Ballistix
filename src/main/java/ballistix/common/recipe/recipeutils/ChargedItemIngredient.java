...
//
	...
	return Arrays.stream(ingredient.getItems()).map(stack -> {
	    ...
	    IItemElectric.setEnergyStored(copy, charge.getJoules());
	    IItemElectric.setMaximumCapacity(copy, charge.getJoules());
	...
	return "items: " + Arrays.toString(ingredient.getItems()) + ", charge: " + charge.toString() + " is strict: "
		+ isStrict;
    }
