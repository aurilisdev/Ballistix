...
public class TileProximityDetector extends GenericTile {
	...
	addComponent(new ComponentElectrodynamic(this, false, true)
		...
		.maxJoules(BallistixConstants.PROXIMITYDETECTOR_USAGEPERTICK * 20));
	...
	AABB box = new AABB(getBlockPos().offset(minCorner.getValue().multiply(-1)),
		getBlockPos().offset(maxCorner.getValue()).offset(1, 1, 1));
	...
	if (usingWhitelist.getValue()) {
		...
		if (inHand.isEmpty()) {
		    ...
		    if (!inHand.isEmpty() && inHand.is(BallistixItems.ITEM_SCANNER.get())) {
			continue;
		    }
		} else if (inHand.is(BallistixItems.ITEM_SCANNER.get())) {
		    continue;
		}
