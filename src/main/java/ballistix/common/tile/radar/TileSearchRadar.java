...
public class TileSearchRadar extends GenericTile {
    ...
    private AxisAlignedBB searchArea = AxisAlignedBB.ofSize(0, 0, 0);
    ...
    public TileSearchRadar() {
        ...
        addComponent(new ComponentContainerProvider("container.searchradar", this).createMenu((id, player) -> new ContainerSearchRadar(id, player, new Inventory(0), getCoordsArray())));
    }
    
    @Override
    public void setLevelAndPosition(World level, BlockPos pos) {
    	super.setLevelAndPosition(level, pos);
    	searchArea = new AxisAlignedBB(pos).inflate(Constants.RADAR_RANGE);
    }

    public void tickServer(ComponentTickable tickable) {
        ComponentElectrodynamic electro = getComponent(IComponentType.Electrodynamic);

        isRunning.set(electro.getJoulesStored() > (Constants.RADAR_USAGE / 20.0) && level.getBrightness(LightType.SKY, getBlockPos()) > 0);
        ...
        for (TileESMTower tile : trackedEsmTowers) {
            detections.add(new IDetected.Detected(new Vector3d(tile.getBlockPos().getX(), tile.getBlockPos().getY(), tile.getBlockPos().getZ()), BallistixBlocks.blockEsmTower.asItem(), false));
        }
    ...
    public void tickClient(ComponentTickable tickable) {
        ...
        clientRotationSpeed = MathHelper.clamp(clientRotationSpeed + 0.25 * (isRunning.get() ? 1 : -1), 0.0, 10.0);
