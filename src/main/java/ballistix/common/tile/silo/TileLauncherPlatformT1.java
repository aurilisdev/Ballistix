...
public class TileLauncherPlatformT1 extends GenericTile implements ILauncherPlatform, IMultiblockParentTile {
    ...
    public TileLauncherPlatformT1(TileEntityType<?> type) {
        super(type);
        int tier = getTier();
    ...
    public int launch(ILauncherControlPanel controlPanel, boolean redstoneTriggered, int inaccuracy) {
        ...
        if (redstoneTriggered && hasSam.getValue()) {
            ...
            TileEntity tileentity = level.getBlockEntity(target);
            ...
            if (
                //
                    tileentity instanceof TileFireControlRadar &&
                            ...
                            //
                            TileFireControlRadar.getDistanceToMissile(new Vector3d(getBlockPos().getX(), getBlockPos().getY(), getBlockPos().getZ()), ((TileFireControlRadar) tileentity).tracking.position) > 100
                //
            ) {
        	...
        	TileEntity tileentity = level.getBlockEntity(controlPanel.getTarget());
    ...
    public boolean launchMissile(BlockPos target, int frequency) {
        ...
        if (mis.getItem() instanceof ItemMissile && explosive.getItem() instanceof BlockItemDescriptable && ((BlockItemDescriptable) explosive.getItem()).getBlock() instanceof BlockExplosive) {
        	ItemMissile itmissile = (ItemMissile) mis.getItem();
        	BlockExplosive blexplosive = (BlockExplosive) ((BlockItem) explosive.getItem()).getBlock();
    ...
    protected boolean isItemValidForSlot(int index, ItemStack stack, ComponentInventory inv) {
        ...
        if (index == 0) {
            ...
            return item instanceof BlockItemDescriptable && ((BlockItemDescriptable) item).getBlock() instanceof BlockExplosive && ((BlockExplosive) ((BlockItemDescriptable) item).getBlock()).explosive.tier <= getTier() && ((BlockExplosive) ((BlockItemDescriptable) item).getBlock()).explosive.tier > -1;
        }
        return false;
    }
    ...
    private void handleExplosive(ComponentInventory inv, int index) {
        if (index == 1 || index == -1) {
            ...
            if ((!explosive.isEmpty() && explosive.getItem() instanceof BlockItemDescriptable && ((BlockItemDescriptable) explosive.getItem()).getBlock() instanceof BlockExplosive) || (explosive.isEmpty() && inv.getItem(MISSILE_SLOT).getItem() == BallistixItems.ITEM_AAMISSILEMK2.get())) {
    ...
    @Override
    public <T> LazyOptional<T> getSubnodeCapability(@Nullable Capability<T> cap, Direction side) {
