...
public class TileTurretLaser extends TileTurretAntimissile implements ITickableSound {
	...
	super(BallistixTiles.TILE_LASERTURRET.get(), worldPos, blockState,
		BallistixConfig.INSTANCE.LASER_TURRET_BASE_RANGE.get(), 0,
		BallistixConfig.INSTANCE.LASER_TURRET_USAGEPERTICK.get(),
		BallistixConfig.INSTANCE.LASER_TURRET_ROTATIONSPEEDRADIANS.get(), 0);
    }
	...
	if (heat.getValue() < BallistixConfig.INSTANCE.LASER_TURRET_COOLTHRESHHOLD.get()) {
	...
	if (heat.getValue() > BallistixConfig.INSTANCE.LASER_TURRET_MAXHEAT.get()) {
	...
	if (overheated.getValue() && level.random.nextDouble() < 0.5) {
	    level.addParticle(ParticleTypes.LARGE_SMOKE, getBlockPos().getX() + level.random.nextDouble(),
		    getBlockPos().getY() + level.random.nextDouble(), getBlockPos().getZ() + level.random.nextDouble(),
		    0, 0, 0);
	}
    }
	...
	if (target != null && canRaycastTo(level, getProjectileLaunchPosition(), target.getTargetLocation())) {
