...
import ballistix.api.damage.DamageSourceLaserTurret;
...
public class TileTurretLaser extends TileTurretAntimissile implements ITickableSound {
	...
	public final Property<Vector3d> targetPos = property(new Property<>(PropertyType.Vec3, "targetposition", TileFireControlRadar.OUT_OF_REACH));
	...
	public TileTurretLaser() {
		super(BallistixBlockTypes.TILE_LASERTURRET.get(), Constants.LASER_TURRET_BASE_RANGE, 0, Constants.LASER_TURRET_USAGEPERTICK, Constants.LASER_TURRET_ROTATIONSPEEDRADIANS, 0);
	}
	...
	public void tickClient(ComponentTickable tickable) {
		if (shouldPlaySound() && !isPlaying) {
			isPlaying = true;
			SoundBarrierMethods.playTileSound(BallistixSounds.SOUND_LASER_TURRETFIRING.get(), SoundCategory.BLOCKS, this, 1.0F, 1.0F, true);
		}
	...
	public Vector3d getProjectileLaunchPosition() {
		...
		return new Vector3d(above.getX() + 0.5, above.getY() + 1.03125, above.getZ() + 0.5);
	}

	@Nullable
	@Override
	public Vector3d getTargetPosition(@Nonnull ITarget target) {
	...
	public @Nullable ITarget getTarget(long ticks) {
		...
		if (ticks % 5 == 0) {
			...
			for (LivingEntity entity : level.getEntitiesOfClass(type, new AxisAlignedBB(getBlockPos()).inflate(currentRange.get() / 4.0))) {
				if (raycastToBlockPos(level, getBlockPos(), entity.blockPosition()).isEmpty() && !(entity instanceof PlayerEntity && (((PlayerEntity) entity).isCreative() || whitelistedPlayers.get().contains(((PlayerEntity) entity).getName().getString()))) && !entity.isDeadOrDying() && !entity.removed) {
