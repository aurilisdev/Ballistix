...
public class TileTurretLaser extends TileTurretAntimissile implements ITickableSound {
    ...
    public final SingleProperty<Vector3d> targetPos = property(new SingleProperty<>(PropertyTypes.VEC3, "targetposition", TileFireControlRadar.OUT_OF_REACH));
    ...
    public TileTurretLaser() {
        super(BallistixTiles.TILE_LASERTURRET.get(), BallistixConstants.LASER_TURRET_BASE_RANGE, 0, BallistixConstants.LASER_TURRET_USAGEPERTICK, BallistixConstants.LASER_TURRET_ROTATIONSPEEDRADIANS, 0);
    }
    ...
    @Nullable
    @Override
    public Vector3d getTargetPosition(@Nullable ITarget target) {
    ...
    public @Nullable ITarget getTarget(long ticks) {
        ...
        if (ticks % 5 == 0) {
            ...
            for (LivingEntity entity : level.getEntitiesOfClass(type, new AxisAlignedBB(getBlockPos()).inflate(currentRange.getValue() / 4.0))) {
                if (raycastToBlockPos(level, getBlockPos(), entity.blockPosition()).isEmpty() && !(entity instanceof PlayerEntity && (((PlayerEntity) entity).isCreative() || whitelistedPlayers.getValue().contains(((PlayerEntity) entity).getName().getString()))) && !entity.isDeadOrDying() && entity.isAlive()) {
