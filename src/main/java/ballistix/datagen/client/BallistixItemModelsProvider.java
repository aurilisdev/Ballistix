...
public class BallistixItemModelsProvider extends ElectrodynamicsItemModelsProvider {
	...
	protected void registerModels() {
		...
		simpleBlockItem(BallistixBlocks.blockFireControlRadar, existingBlock(blockLoc("firecontrolradarfull"))).transforms()
				...
				.transform(Perspective.FIRSTPERSON_RIGHT).rotation(0, 45, 0).translation(0, 0, 0).scale(0.40F).end()
				//
				.transform(Perspective.THIRDPERSON_RIGHT).rotation(75, 225, 0).translation(0, 2.5F, 0).scale(0.375F).end()
				//
				.transform(Perspective.FIRSTPERSON_RIGHT).rotation(0, 225, 0).translation(0, 0, 0).scale(0.40F).end();

		simpleBlockItem(BallistixBlocks.blockSamTurret, existingBlock(blockLoc("samturretitem")));
		simpleBlockItem(BallistixBlocks.blockEsmTower, existingBlock(blockLoc("esmtower"))).transforms().transform(Perspective.GUI).scale(0.3F).rotation(30.0F, 225.0F, 0.0F).end();
