...
public class ScreenComponentBallistixButton<T extends ScreenComponentBallistixButton<?>> extends ScreenComponentGeneric {
    ...
    public void renderBackground(PoseStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
    	if (isVanillaRender && isVisible()) {
			...
			RenderingUtils.bindTexture(AbstractWidget.WIDGETS_LOCATION);
			blit(stack, this.xLocation + guiWidth, this.yLocation + guiHeight, 0, 46 + i * 20, this.width / 2, this.height);
			blit(stack, this.xLocation + guiWidth + this.width / 2, this.yLocation + guiHeight, 200 - this.width / 2, 46 + i * 20, this.width / 2, this.height);
			...
			if (this.icon != null) {
                ...
                blit(stack, guiWidth + this.xLocation + xOffset, guiHeight + this.yLocation + yOffset, (float)this.icon.textureU(), (float)this.icon.textureV(), this.icon.textureWidth(), this.icon.textureHeight(), this.icon.imageWidth(), this.icon.imageHeight());
            }

			Font font = minecraft.font;
			if (label != null) {
				drawCenteredString(stack, font, label.get(), this.xLocation + guiWidth + this.width / 2, this.yLocation + guiHeight + (this.height - 8) / 2, color.color());
			}
            ...
            if(icon != null) {
    			...
    			RenderingUtils.bindTexture(this.icon.getLocation());
    			blit(stack, guiWidth + xLocation + xOffset, guiHeight + yLocation + yOffset, icon.textureU(), icon.textureV(), icon.textureWidth(), icon.textureHeight(), icon.imageWidth(), icon.imageHeight());
    		}
        }
    ...
    public static interface OnTooltip {
		...
		public void onTooltip(PoseStack stack, ScreenComponentBallistixButton<?> button, int xAxis, int yAxis);
