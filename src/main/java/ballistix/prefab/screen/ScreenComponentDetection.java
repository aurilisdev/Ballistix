...
public class ScreenComponentDetection extends ScreenComponentGeneric {
	...
	Component text = Component.literal(new BlockPos((int) detection.getPosition().x,
		(int) detection.getPosition().y, (int) detection.getPosition().z).toString());
	...
	if (detection.showBearing()) {
	    ...
	    text = BallistixTextUtils
		    .gui("radar.bearing", Component.literal("" + thetaMin % 360).withStyle(ChatFormatting.WHITE),
			    Component.literal("" + thetaMax % 360).withStyle(ChatFormatting.WHITE))
	    ...
	    graphics.drawString(font, BallistixTextUtils.gui("radar.bearingunknown"), guiWidth + x, guiHeight + y,
