...
public class ScreenComponentRadarGrid extends ScreenComponentGeneric {
	...
	for (int i = 1; i < 10; i++) {
	    ...
	    fill(poseStack, x + 1, y + gridWidth * i, x + this.width - 1, y + 1 + gridWidth * i,
	...
	fill(poseStack, (int) Math.floor(x + 1 - extra - 2), (int) Math.floor(y + center - 1),
		(int) Math.ceil(x + center), (int) Math.ceil(y + center + 1), RADAR_PULSE_GREEN.color());
	...
	fill(poseStack, (int) Math.floor(x + center - 1), (int) Math.floor(y + center - 1),
	...
	for (int i = 0; i < missiles.size(); i++) {
	    ...
	    float deltaX = (pos.getX() - tile.getBlockPos().getX())
		    / (2.0f * BallistixConstants.FIRE_CONTROL_RADAR_RANGE) * width;
	    ...
	    fill(poseStack, dotX, dotY, dotX + 1, dotY + 1, colour);
	}
    }
}
