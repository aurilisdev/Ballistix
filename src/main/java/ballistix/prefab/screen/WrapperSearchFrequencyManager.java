...
public class WrapperSearchFrequencyManager {
    ...
    public WrapperSearchFrequencyManager(ScreenSearchRadar screen, int tabX, int tabY, int x, int y) {
        ...
        screen.addComponent(button = (ScreenComponentBallistixButton<?>) new ScreenComponentBallistixButton<>(ScreenComponentGuiTab.GuiInfoTabTextures.REGULAR, tabX, tabY).setOnPress(button -> {
            ...
            tooltips.add(BallistixTextUtils.tooltip("radar.frequencymanager").withStyle(TextFormatting.DARK_GRAY));
            if (!button.isPressed) {
                tooltips.add(ElectroTextUtils.tooltip("inventoryio.presstoshow").withStyle(TextFormatting.ITALIC, TextFormatting.GRAY));
            } else {
                tooltips.add(ElectroTextUtils.tooltip("inventoryio.presstohide").withStyle(TextFormatting.ITALIC, TextFormatting.GRAY));
            }
