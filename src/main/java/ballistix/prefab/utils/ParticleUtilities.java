...
public class ParticleUtilities {
	...
	public static void spawnParticleRing(IParticleData particle, double centerX, double centerY, double centerZ, int count, double initialSpeed, boolean shouldRandomize) {
		for (float rad = 0; rad < 2.0 * Math.PI; rad += 2 * Math.PI / count) {
			double x = MathHelper.cos(rad) * (shouldRandomize ? Voltaic.RANDOM.nextDouble() * 0.05 + 1 : 1) * initialSpeed;
			double z = MathHelper.sin(rad) * (shouldRandomize ? Voltaic.RANDOM.nextDouble() * 0.05 + 1 : 1) * initialSpeed;
			Minecraft.getInstance().particleEngine.createParticle(particle, centerX, centerY, centerZ, x, 0, z);
		}
	}

	public static void spawnParticleSphere(IParticleData particle, double centerX, double centerY, double centerZ, int count, double thetamin, double thetamax, double initialSpeed, boolean shouldRandomize) {
	 ...
	 */
	public static double progressGroundShockwave(World world, double x, double z, double progress, double spawnSize, double endSize, double chance) {
		if (progress > 1)
			return endSize;
		...
		for (double i = 0; i < 360.0; i += 360.0 / (Math.PI * 2 * r) * (0.5 + Voltaic.RANDOM.nextDouble())) {
			if (Voltaic.RANDOM.nextDouble() < chance) {
				...
				int rx = (int) (x + r * dirX);
				int rz = (int) (z + r * dirZ);
				int ry = world.getHeight(Type.WORLD_SURFACE, rx, rz);
